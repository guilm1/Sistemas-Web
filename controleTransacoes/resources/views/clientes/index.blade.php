...
<html lang="br" dir="ltr">
  <head>
    <meta charset="utf-8">
    <title>@yield('titulo', 'Gerencia Clientes')</title>
  </head>
  ...
  <body>
    ...
    <div class="jumbotron">
      ...
      <h3>Efetue seu cadastro para ter acesso!</h3>
    </div>

    <div class="btn btn-light">
        <a class="btn btn-link" href="{{ route('clientes.create') }}">Inserir novo Cliente</a>
    </div>

    <table class=" table table-bordered table-hover table-striped">
      <caption><p>Todos os clientes ativos!</p> </caption>
    ...
    @foreach($clientes as $c)
      <tr>
        ...
        <td>{{$c->nome}}</td>
        <td>{{$c->created_at}}</td>
        <td><a class="btn btn-success" href="{{ route('clientes.edit', $c->id) }}">Exibir</a></td>
        <td>
          <form method="post" action="{{ route('clientes.destroy', $c->id) }}" onsubmit="return confirm('Confirma exclusão do cliente?');" >
